...
lxd_opts = [
    ...
    cfg.StrOpt("lxd_share_prefix",
               default="manila_share",
               help="Prefix to be used when creating a share."),
]
...
class LXDBaseHelper(object):
    ...
    def _get_share_name(self):
        name = "%(prefix)s_%(id)s"
        expd = {
            "prefix": self.conf.lxd_share_prefix,
            "id": six.text_type(self.share['id']),
        }
        return name % expd


class LXDUnfs3Helper(LXDBaseHelper):
    ...
    # production.
    def __init__(self, lxd_helper, *args, **kwargs):
        super(LXDUnfs3Helper, self).__init__(lxd_helper, *args, **kwargs)
    ...
    def create_share(self, share, server_id):
        ...
        location = result + ':' + "/shares/%s" % self._get_share_name(share)
        return location

    def delete_share(self, share, server_id):
    ...
    def _deny_access(self, share, server_id, host_to_deny):
        share_name = self._get_share_name(share)
        share_folder = "/shares/%s" % share_name
    ...
    def _allow_access(self, share, server_id, host_to_allow,
                      access_level):
    ...
    def update_access(self, share, server_id, access_rules, add_rules,
                      delete_rules):
        ...
        if not (add_rules or delete_rules):
            ...
            for rule in (access_rules or []):
                ...
                if access_type == 'ip':
                    self._allow_access(share, server_id, host_to_allow,
                                       access_level)
                else:
        ...
        for rule in delete_rules:
            ...
            access_type = rule['access_type']
            if access_type == 'ip':
                self._deny_access(share, server_id, host_to_deny)
            else:
                LOG.warning(_LW("Attempt to use access type %s has been "
                                "blocked.") % access_type)


class LXDCIFSHelper(LXDBaseHelper):
    # NOTE(aovchinnikov): current implementation limits CIFS access rules
    # to 'user' only. 'ip' authentication has to be re-added later. It is
    def __init__(self, lxd_helper, *args, **kwargs):
        super(LXDCIFSHelper, self).__init__(lxd_helper, *args, **kwargs)
    ...
    def create_share(self, share, server_id):
        ...
        location = '\\\\' + result + '\\' + share_name
        return location

    def delete_share(self, share, server_id):
        self.lxd.execute_sync(
            server_id,
            ["net", "conf", "delshare", self._get_share_name(share)]
        )
    ...
    def _deny_user_access(self, share, server_id, user_to_deny):
        share_name = share.share_id
        for access in ('read list', 'valid users'):
            allowed_users = self.lxd.execute_sync(
                server_id,
                ["net", "conf", "getparm", share_name, access]
            )
            pruned_users = filter(lambda x: not x.startswith(user_to_deny),
                                  allowed_users.split())
            allowed_users = " ".join(pruned_users)
            self.lxd.execute_sync(
                server_id,
    ...
    def _allow_user_access(self, share, server_id, user_to_allow,
                           access_level):
        share_name = share.share_id
        if access_level == const.ACCESS_LEVEL_RO:
            access = 'read list'
    ...
    def update_access(self, share, server_id, access_rules,
        ...
        for rule in add_rules:
            ...
            if access_type == 'user':
                ...
                raise exception.InvalidShareAccess(reason=msg)
        for rule in delete_rules:
            entity_to_deny = rule['access_to']
            access_type = rule['access_type']
            if access_type == 'user':
                self._deny_user_access(share_name, server_id, entity_to_deny)
            else:
                LOG.warning(_LW("Attempt to use access type %s has been "
                                "blocked.") % access_type)
...
class LXDDriver(driver.GaneshaMixin, driver.ShareDriver, driver.ExecuteMixin):
    ...
    def _get_lv_device(self, share):
        return os.path.join("/dev", self.configuration.lxd_volume_group,
                            self._get_helper(share)._get_share_name())
    ...
    def _get_lv_folder(self, share):
        ...
        return os.path.join("/tmp", self._get_helper(share)._get_share_name())

    def create_share(self, context, share, share_server=None):
        LOG.debug("Create share on server '%s'." % share_server["id"])
        server_id = self._get_container_name(share_server["id"])
        share_name = self._get_helper(share)._get_share_name()
        self.lxd.execute_sync(
            server_id,
        ...
        location = self._get_helper(share).create_share(share, server_id)
        return location
    ...
    def delete_share(self, context, share, share_server=None):
        ...
        self.lxd.execute_sync(
            server_id,
            ["rm", "-fR",
             "/shares/%s" % self._get_helper(share)._get_share_name()]
        )
        LOG.debug("Deletion of share %s is complete!", share.id)
    ...
    def update_access(self, context, share, access_rules, add_rules,
        ...
        server_id = self._get_container_name(share_server["id"])
        share_name = self._get_helper(share)._get_share_name(share)
        LOG.debug("Updating access to share %(share)s at "
                  "share server %(share_server)s.",
                  {"share_server": share_server["id"],
                   "share": share.id})
        self._get_helper(share).update_access(share, server_id,
    ...
    def _check_for_setup_errors(self):
        host_id = self.configuration.safe_get("neutron_host_id")
        neutron_class = ('manila.network.neutron.neutron_network_plugin.'
                         'NeutronNetworkPlugin')
        actual_class = self.configuration.safe_get("network_api_class")
        if host_id is None and (actual_class == neutron_class):
            msg = _("%s requires neutron_host_id to be "
                    "specified.") % neutron_class
            raise exception.ManilaException(msg)
        elif host_id is None:
            LOG.info(_LI("neutron_host_id is not specified. This driver "
                         "might not work as expected without it."))

    def _setup_server(self, network_info, metadata=None):
        msg = "Creating share server '%s'."
        self._check_for_setup_errors()

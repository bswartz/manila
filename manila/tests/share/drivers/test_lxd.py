...
@testtools.skipIf(lxd.NO_LXD, "pylxd is unavailable")
class LXDBaseHelperTestCase(test.TestCase):
    """Tests LXDUnfs3Helper"""

    def setUp(self):
        super(LXDBaseHelperTestCase, self).setUp()
        fake_config = mock.Mock()
        fake_config.lxd_share_prefix = "prefix"
        fake_lxd = mock.Mock()
        fake_share = {"id": "fakeid"}
        self.basehelper = lxd.LXDBaseHelper(fake_lxd, share=fake_share,
                                            config=fake_config)

    def tearDown(self):
        super(LXDBaseHelperTestCase, self).tearDown()

    def test__get_share_name(self):
        self.assertEqual("prefix_fakeid", self.basehelper._get_share_name())
...
class LXDUnfs3HelperTestCase(test.TestCase):
    ...
    def test_create_share(self):
        ...
        self.UNFS3Helper.create_share(fake_share(), 'fakeserver')
    ...
    def test_delete_share(self):
        ...
        self.mock_object(self.lxd_helper, 'execute_sync')
        self.UNFS3Helper.delete_share(fake_share(), 'fakeserver')
        self.UNFS3Helper.lxd.execute_sync.assert_called_once_with(
            'fakeserver',
            ['sed', '-i', '\\$/shares/manila_share_fakeid.*$d',
             '/etc/exports'])
    ...
    def test__deny_access(self):
        self.mock_object(self.UNFS3Helper, '_restart_unfsd')
        self.mock_object(self.lxd_helper, 'execute_sync')
        self.UNFS3Helper._deny_access(fake_share(), 'fakeserver',
                                      '127.0.0.1')
        self.UNFS3Helper.lxd.execute_sync.assert_called_once_with(
            'fakeserver',
            ['sed', '-i', '\\$/shares/manila_share_fakeid.*127\\.0\\.0'
             '\\.1.*$d', '/etc/exports'])
        self.UNFS3Helper._restart_unfsd.assert_called_once_with('fakeserver')
    ...
    def test__allow_access_host_present_ro(self):
        ...
        expected_arguments = [
            ('fakeserver',
             ['grep', '/shares/manila_share_fakeid.*127\\.0\\.0\\.1.*',
              '/etc/exports'])]
        self.lxd_helper.execute_sync = functools.partial(
            self.fake_exec_sync, execute_arguments=actual_arguments,
            ret_val='127.0.0.1')
        self.UNFS3Helper._allow_access(fake_share(), 'fakeserver', '127.0.0.1',
                                       'rw')
        self.assertEqual(expected_arguments, actual_arguments)
    ...
    def test__allow_access_no_host_ro(self):
        ...
        expected_arguments = [
            ...
            ('fakeserver',
             ['sed', '-i',
              '$ a\\/shares/manila_share_fakeid 127.0.0.1(rw,no_root_squash,'
              'async,no_subtree_check)', '/etc/exports'])]
        self.lxd_helper.execute_sync = functools.partial(
            self.fake_exec_sync, execute_arguments=actual_arguments,
            ret_val='')
        self.mock_object(self.UNFS3Helper, '_restart_unfsd')
    ...
    def test_update_access_access_rules_ok(self):
        ...
        self.UNFS3Helper.update_access(share, "fakeserver",
                                       allow_rules, [], [])
        self.UNFS3Helper._allow_access.assert_called_once_with(share,
                                                               "fakeserver",
                                                               "127.0.0.1",
                                                               "ro")
    ...
    def test_update_access_access_rules_wrong_type(self):
        ...
        self.assertRaises(exception.InvalidShareAccess,
                          self.UNFS3Helper.update_access, fake_share(),
    ...
    def test_update_access_delete_rules_ok(self):
        ...
        self.UNFS3Helper.update_access(share, "fakeserver", [], [],
                                       delete_rules)
        self.UNFS3Helper._deny_access.assert_called_once_with(share,
                                                              "fakeserver",
                                                              "127.0.0.1")
    ...
    def test_update_access_delete_rules_not_ok(self):
        ...
        self.mock_object(self.UNFS3Helper, "_deny_access")
        self.UNFS3Helper.update_access(fake_share(), "fakeserver", [], [],
                                       delete_rules)
...
class LXDCIFSHelperTestCase(test.TestCase):
    ...
    def setUp(self):
        ...
        self.fake_conf.lxd_share_prefix = "prefix"
    ...
    def test_create_share_guest_ok(self):
        ...
        self.lxd_helper.execute_sync = functools.partial(
            self.fake_exec_sync, execute_arguments=actual_arguments,
            ret_val="0/0 " * 20)
        self.CIFSHelper.create_share(fake_share(), "fakeserver")
    ...
    def test_create_share_guest_not_ok(self):
        ...
        self.lxd_helper.execute_sync = functools.partial(
            ...
            ret_val="0/0 " * 20)
        share = fake_share()
        self.CIFSHelper.create_share(share, "fakeserver")
        self.assertEqual(expected_arguments.sort(), actual_arguments.sort())

    def test_delete_share(self):
        share = fake_share()
        self.CIFSHelper.delete_share(share, "fakeserver")
        self.CIFSHelper.lxd.execute_sync.assert_called_with(
            'fakeserver',
            ['net', 'conf', 'delshare', 'prefix_fakeid'])

    def test__deny_access_host_present(self):
        self.lxd_helper.execute_sync.side_effect = ['127.0.0.1', ""]
        share = fake_share()
        self.CIFSHelper._deny_access(share, "fakeserver", "127.0.0.1")
        self.CIFSHelper.lxd.execute_sync.assert_called_with(
            'fakeserver',
            ['net', 'conf', 'setparm', 'prefix_fakeid', 'hosts allow', ''])
    ...
    def test__deny_access_no_host(self):
        ...
        self.CIFSHelper.lxd.execute_sync.assert_called_with(
            'fakeserver',
            ['net', 'conf', 'getparm', 'prefix_fakeid', 'hosts allow'])

    def test__allow_access_host_present(self):
        self.lxd_helper.execute_sync.side_effect = ['127.0.0.1', ""]
        self.CIFSHelper._allow_access(fake_share(), "fakeserver", "127.0.0.1",
                                      "rw")
        self.CIFSHelper.lxd.execute_sync.assert_called_with(
            'fakeserver',
    ...
    def test_allow_access_ro_guest_not_ok(self):
        ...
        self.CIFSHelper.conf.lxd_cifs_guest_ok = "no"
        self.assertRaises(exception.ManilaException,
                          self.CIFSHelper._allow_access, fake_share(),
                          "fakeserver", "127.0.0.1", "ro")

    def test_allow_user_access_ok(self):
        self.CIFSHelper._allow_user_access(fake_share(), "fakeserver",
                                           "fakeuser", "ro")
        self.CIFSHelper.lxd.execute_sync.assert_called_with(
            'fakeserver',
    ...
    def test_allow_user_access_not_ok(self):
        self.assertRaises(exception.InvalidShareAccessLevel,
                          ...
                          fake_share(), "fakeserver", "fakeuser", "rx")
    ...
    def test_update_access_access_rules_ok_user(self):
        ...
        self.CIFSHelper.update_access(share, "fakeserver", allow_rules,
                                      [], [])
    ...
    def test_update_access_access_rules_wrong_type(self):
        ...
        self.assertRaises(exception.InvalidShareAccess,
                          self.CIFSHelper.update_access, fake_share(),
    ...
    def test_update_access_add_rules_ok_user(self):
        ...
        self.mock_object(self.CIFSHelper, "_allow_user_access")
        self.CIFSHelper.update_access(fake_share(), "fakeserver", [],
                                      add_rules, [])
        self. CIFSHelper._allow_user_access.assert_called_once_with(
            "fakeshareid",
            "fakeserver",
    ...
    def test_update_access_delete_rules_ok(self):
        ...
        self.mock_object(self.CIFSHelper, "_deny_user_access")
        self.CIFSHelper.update_access("fakeshareid", "fakeserver", [], [],
                                      delete_rules)
        self.CIFSHelper._deny_user_access.assert_called_once_with(
            "fakeshareid",
            "fakeserver",
            "127.0.0.1"
        )

    def test__deny_user_access(self):
        expected_arguments = [
            ('fakeserver', ['net', 'conf', 'getparm', 'fakeshareid',
             'read list']),
            ('fakeserver', ['net', 'conf', 'setparm', 'fakeshareid',
             'read list', '']),
            ('fakeserver', ['net', 'conf', 'getparm', 'fakeshareid',
        ...
        self.CIFSHelper._deny_user_access("fakeshareid", "fakeserver",
                                          "fake-user")
        self.assertEqual(expected_arguments.sort(), actual_arguments.sort())
...
class LXDDriverTestCase(test.TestCase):
    ...
    def test_create_share(self):
        helper = mock.Mock()
        helper._get_share_name = mock.Mock(return_value="whatever")
    ...
    def test__get_helper_nfs_new(self):
        share = fake_share(share_proto='NFS')
    ...
    def test__get_helper_nfs_existing(self):
        share = fake_share(share_proto='NFS')
        fake_nfs_helper = mock.Mock()
        self._driver._helpers['NFS'] = fake_nfs_helper
    ...
    def test__get_helper_cifs_existing(self):
        share = fake_share(share_proto='CIFS')
        fake_cifs_helper = mock.Mock()
        self._driver._helpers['CIFS'] = fake_cifs_helper
        result = self._driver._get_helper(share)
        self.assertEqual(fake_cifs_helper, result)
    ...
    def test__get_lv_device(self):
        self.assertEqual("/dev/manila_lxd_volumes/prefix_fakeid",
                         self._driver._get_lv_device(self.share))

    def test__get_lv_folder(self):
        self.assertEqual("/tmp/prefix_fakeid",
    ...
    def test_delete_share(self):
        helper = mock.Mock()
        helper._get_share_name = mock.Mock(return_value="prefix_fakeid")
        self.mock_object(self._driver, "_get_helper",
                         mock.Mock(return_value=helper))
        self.mock_object(self._driver, '_execute')
        self.mock_object(self._driver.lxd, 'execute_sync')
        ...
        self._driver._execute.assert_called_with(
            ...
            '/dev/manila_lxd_volumes/prefix_fakeid',
            run_as_root=True)
    ...
    def test_update_access_access_rules_ok(self):
        ...
        self._driver.update_access(self._context, self.share,
                                   ...
                                   [], [], {"id": "fake"})
        helper.update_access.assert_called_with('prefix_fakeid', 'manila-fake',
                                                [{'access_level': 'rw'}],
                                                [], [])
    ...
    def test__setup_server_ok(self):
        def fake_safe_get(opt):
            if opt == "neutron_host_id":
                return "fake-neutron-host"
            return None
        network_info = fake_network()
        self.mock_object(self._driver.configuration, "safe_get", fake_safe_get)
        self.mock_object(lxd.LOG, 'warning')
        ...
        self._driver._connect_to_network.assert_called_once_with(server_id,
                                                                 network_info)
        self.assertFalse(lxd.LOG.warning.called)

    def test__check_for_setup_errors_ok(self):
        def fake_safe_get(opt):
            if opt == "neutron_host_id":
                return "fake-host-id"
            return None
        self.mock_object(self._driver.configuration, "safe_get", fake_safe_get)
        self.mock_object(lxd.LOG, 'info')
        self._driver._check_for_setup_errors()
        self.assertFalse(lxd.LOG.info.called)

    def test__check_for_setup_errors_no_host_id(self):
        def fake_safe_get(opt):
            if opt == "neutron_host_id":
                return None
            elif opt == "network_api_class":
                return "fake.class"
            return None
        self.mock_object(self._driver.configuration, "safe_get", fake_safe_get)
        self.mock_object(lxd.LOG, 'info')
        self._driver._check_for_setup_errors()
        self.assertTrue(lxd.LOG.info.called)

    def test__check_for_setup_errors_raises(self):
        self.assertRaises(exception.ManilaException,
                          self._driver._check_for_setup_errors)
